(function () {
  ...
  function drawCanvas(phaseIndex) {
    ...
    if (activeCase && activeCase.targets && phaseIndex === 0) {
      const firstKey = phase.sliders[0].id;
      const target = activeCase.targets[firstKey];
    ...
    for (let x = 0; x < canvas.width; x++) {
      ...
      let y = baseline;
      if (mode === 'bone') {
        y = canvas.height - (mean / 100) * (canvas.height - 20) + Math.sin(t) * 4;
      } else if (mode === 'cardio') {
        const beat = Math.max(0, Math.sin(t * (0.8 + vals[0] / 80)));
        y = baseline - Math.pow(beat, 6) * (40 + vals[1] * 0.4) - 20;
      } else if (mode === 'resp') {
        y = baseline - 35 - Math.sin(t * (0.6 + vals[0] / 90)) * (18 + vals[1] * 0.25);
      } else if (mode === 'fick') {
        const climb = Math.min(1, x / (canvas.width * 0.7));
        y = baseline - climb * (mean * 0.9) + Math.sin(t) * 3;
      } else {
        y = canvas.height - (mean / 100) * canvas.height + Math.sin(t) * 6;
      }
  ...
  window.updateLabPreview = function (phaseIndex) {
    const phase = L.phases[phaseIndex];
    phase.sliders.forEach(s => {
      const el = $('val-' + s.id);
      if (el) el.innerText = sliderValue(s.id);
    });
    if (phase.status) {
  ...
  function checkPhase(phaseIndex) {
    ...
    if (activeCase.rules && activeCase.rules[phaseIndex]) {
      const msg = activeCase.rules[phaseIndex](vals);
      if (msg) return msg;
    }
  ...
  window.initLabSession = function () {
    ...
    $('lock-1').className = 'padlock-status locked';
    $('lock-1').innerText = 'Cadeado Fechado';
    $('lock-2').className = 'padlock-status locked';
    $('lock-2').innerText = 'Cadeado Fechado';
    document.querySelectorAll('input, select, button').forEach(el => el.disabled = false);
    $('p1-objective').innerHTML = '<strong>' + activeCase.title + '</strong><br><br>' + activeCase.desc +
      '<br><br><em>Urgência:</em> ' + (timerEnabled
        ? ('Cronômetro ativo com <strong>' + timeLeft + 's</strong> por fase.')
        : 'Modo de análise livre ativado.');
    L.phases.forEach(phase => {
      phase.sliders.forEach(s => { $(s.id).value = s.value; });
    });
    updateLabPreview(0);
  ...
  window.validatePhase1 = function () {
    ...
    $('lock-1').className = 'padlock-status unlocked';
    $('lock-1').innerText = 'Cadeado Aberto';
  ...
  window.validatePhase2 = function () {
    ...
    $('lock-2').className = 'padlock-status unlocked';
    $('lock-2').innerText = 'Cadeado Aberto';
    $('phase-2-card').style.opacity = '0.6';
    $('global-status').innerText = 'Sucesso Total';
    $('global-status').style.color = 'var(--success)';
    showModal('success', '🏆', 'Parabéns! Missão concluída',
      'O estudo de caso para <strong>' + activeCase.title + '</strong> foi validado.<br><br>🔓 <strong>Cadeado Aberto e Caso Salvo!</strong>');
  };
})();
